  ...
  CouchDB.prototype.migrateOrUpdateIndex = function(models, isMigrate, cb) {
    ...
    function autoUpdateOneModel(model, cb) {
      ...
      function createIndex(name, fields, cb) {
        ...
        //  gets the database as defined by the specific model, or uses the one defined in the
        //  datasource connector
        var database = mo.mo.model.settings.couchdb.database || self.settings.database;
        ...
        self.createIndex(config.ddocName, config.indexName, config.fields, database, cb);
      }
 ...
 * @param {String} name of couchdb database to use
 * @callback {Function} cb The callback function
 */
  CouchDB.prototype.createIndex = function(ddocName, indexName, fields, database, cb) {
      debug('createIndex: ddocName %s, indexName %s, fields %s, database %s', ddocName,
        indexName, fields, database);
    ...
    if (typeof database !== 'string') return cb(new Error('database must be a string!'));
    ...
    var requestObject = {
      ...
      const cookie = self.settings.authSession;
      if (!!cookie) {
